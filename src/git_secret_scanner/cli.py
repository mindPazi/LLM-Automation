...
def main() -> int:
    ...
    try:
        ...
        for i, commit in enumerate(commits):
            ...
            if commit_msg_lines:
                logger.info(f"Processing commit message: {commit.message[:50]}...")
                try:
                    commit_msg_secrets = 0
                    if args.mode == 'llm-only' and llm_analyzer:
                        commit_msg_secrets = llm_analyzer.process_llm_only(commit_msg_lines, commit, "COMMIT_MESSAGE", 
                                                      report_generator, args.model)
                    elif args.mode == 'heuristic-only':
                        commit_msg_secrets = heuristic_filter.process_heuristic_only(commit_msg_lines, commit, 
                    ...
                    if commit_msg_secrets > 0:
                        ...
                        secrets_found_in_commit += commit_msg_secrets
